...
## Data sources
- The crash data is from the UW TOPS lab: https://topslab.wisc.edu/. This is not public, you need to request access to this data.
- School location data is from the Wisconsin Department of Public Instruction: https://data-wi-dpi.opendata.arcgis.com/
- County data is from the US census
- Basemaps are from StadiaMaps: https://stadiamaps.com/

## Example figures
### A map of crashes near Memorial High School in Madison, WI:
...
- School Crash Maps `make schoolpdfs`: generates maps of all the schools in Wisconsin and where cars are hitting kids
    - This script pulls in the TOPS data from a folder of csv’s that you’ve downloaded from the data retrieval tool.
    - It draws the walk boundary around each school, this is done with a OpenStreetMap routing engine that is running in a docker container.
    - It also needs access to an API key for StadiaMaps to download all the basemap tiles. 
    - You can edit file `parameters/run_parameters` to choose what county, school type, and district to generate maps for.
- Dynamic Map `make crashmaps_dynamic`: This takes the TOPS data and generates dynamic leaflet maps to host on the [Wisconsin Bike Fed website](https://wisconsinbikefed.org/what-we-do/advocacy/wisconsin-bike-pedestrian-crash-map/). It generates 4 maps: without a title (for in a frame), and with a title, in both English and Spanish.
