...
# enrollment <- read_csv(file = "data/Schools/enrollment_by_gradelevel_certified_2024-25/enrollment_by_gradelevel_certified_2024-25.csv",
...
bikefed_logo <- image_read_svg(path = "icons/BWF_Vertical_4C.svg")
...
if(str_to_lower(district_focus) == "all") {
  district_focus <- unique(WI_schools |> pull(DISTRICT))
}

school_number <- length(unique(WI_schools |> filter(str_to_upper(COUNTY) %in% county_focus,  
                                              SCHOOLTYPE %in% school_type_focus, 
                                              DISTRICT %in% district_focus) |>  
                          pull(NCES_CODE)))
```
...
get_logo_bounds <- function(center_lon, center_lat, height_meters, image) {
  ...
  # 1. Scale by image aspect ratio
  # 2. Adjust for latitude distortion
...
generate_school_maps <- function(district) {
  ...
  for(school in WI_schools |>
    ...
    logo_height_pct <- 0.16        # Logo height as % of map height
    offset_from_right_pct <- 0.05  # Gap between map edge and logo
    
    # Calculate map dimensions
    map_height_deg <- bbox['top'] - bbox['bottom']
    map_width_deg <- bbox['right'] - bbox['left']
    
    # Calculate logo height in meters
    logo_height_meters <- (map_height_deg * logo_height_pct) * 111320
    
    # Calculate center latitude
    logo_center_lat <- bbox['top'] - map_height_deg * (logo_height_pct / 2)
    
    # Calculate the logo width in degrees
    logo_height_deg <- logo_height_meters / 111320
    ...
    logo_center_lon <- bbox['right'] + map_width_deg * offset_from_right_pct + logo_width_deg / 2
...
#      scale_color_manual(values = bike_lts_scale$color, name = "Bike Level of Traffic Stress") +
      ...
      scale_color_manual(values = "black", name = NULL) +
      # add school location
      # geom_sf(data = school_data, inherit.aes = FALSE) +
      ...
      geom_sf_label(data = school_data,
                    inherit.aes = FALSE,
                    mapping = aes(label = paste(SCHOOL, "School")),
                    nudge_y = 0.0015,
                    label.size = 0.04,
                    size = 2) +
...
double_check <- list()
for(school in WI_schools$NCES_CODE) {
