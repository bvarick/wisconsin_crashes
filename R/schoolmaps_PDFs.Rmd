...
library(cowplot)
library(furrr)
run_parameters <- read_yaml("parameters/run_parameters.yaml")
```
...
## Set parameters of run and filter schools
...
# remove extra "School" in school names
WI_schools <- WI_schools |> 
  mutate(SCHOOL = str_remove(SCHOOL, " School$")) |> 
  mutate(SCHOOL = paste0(SCHOOL, " School"))
```
...
generate_school_maps <- function(district) {
  ...
  for(school in WI_schools |>
    ...
    # Add Bike Fed logo
    school_map <- cowplot::ggdraw(school_map) +
      cowplot::draw_image(
        bikefed_logo, 
        x = 0.9, 
        y = 1.4,
